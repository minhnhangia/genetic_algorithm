...
def mutate_sensor_layout(individual: Individual) -> tuple[Individual]:
    ...
    - 33.3% chance: DROP a random sensor (if >1 sensor exists)
    - 33.3% chance: ADD a new sensor at an unoccupied node (if <4 sensors exist)
    - 33.3% chance: JITTER the pitch/roll/yaw of a random sensor within mechanical limits (±90° pitch, ±90° roll, ±180° yaw)
    """
    mutation_choice = random.random()
    occupied_nodes = set(gene.node_id for gene in individual)
    
    if mutation_choice < 0.333 and len(individual) > 1:
        # DROP MUTAION: Pop a random sensor (safeguarded > 1)
        idx = random.randrange(len(individual))
        individual.pop(idx)
        
    elif 0.333 <= mutation_choice < 0.666 and len(individual) < 4:
